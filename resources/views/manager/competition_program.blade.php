...
<div class="cols">
    @foreach ($buckets as $bucket)
        <div class="mx-8 w-36 grid-cols-4 gap-4">
            @foreach ($bucket['R1'] as $item)
                <div
                    class="border-2 ring-2 bg-gradient-to-r from-slate-800 to-slate-300 text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                    <span class="flex justify-start  p-1 text-white text-[12px]">{{ $item->t_name }}</span>
                    <img class="rounded-full bg-[#ffff] w-[50px] absolute inset-y-0 left-[80px]"
                        style="border-radius: 0% 0% 0% 85% / 0% 0% 100% 100%;" src="{{ $item->logo }}">
                    <div
    ...
    @if ($itemCount % 2 != 0)
        <div class="flex justify-center mt-48">
            <form action="{{ route('randomizeBlindR3', $item->cp_id) }}" method="GET">
                <button type="submit"
                    class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-1 px-2 rounded">ทีมสุ่ม</button>
            </form>
        </div>
    @endif
</div>
            ...
            @foreach ($bucket['R3'] as $index => $item)
                @switch($item->cl_round)
                    @case('1')
                        @if ($item->competition_amount == 8)
                            @if ($item->score == 1)
                                <img src="https://avatars.githubusercontent.com/u/108270321?v=4"
                                    class="object-cover w-6 h-6 rounded-md mt-[-15px]">
                            @endif
                            <div
                                class="border-2 ring-2  @if ($item->score == 1) bg-yellow-500 @else bg-gradient-to-r from-slate-800 to-slate-300 @endif text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                            @else
                                <div
                                    class="border-2 ring-2 bg-gradient-to-r from-slate-800 to-slate-300 text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                        @endif
                    ...
                    @case('3')
                        @if ($item->competition_amount == 8)
                            ...
                            @endif
                            <div
                                class="border-2 ring-2  @if ($item->score == 2) bg-yellow-500 @else bg-gradient-to-r from-slate-800 to-slate-300 @endif text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                            @else
                                <div
                                    class="border-2 ring-2 bg-gradient-to-r from-slate-800 to-slate-300 text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                        @endif
                    ...
                    @case('5')
                        @if ($item->competition_amount == 8)
                            ...
                            @endif
                            <div
                                class="border-2 ring-2  @if ($item->score == 3) bg-yellow-500 @else bg-gradient-to-r from-slate-800 to-slate-300 @endif text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                            @else
                                <div
                                    class="border-2 ring-2 bg-gradient-to-r from-slate-800 to-slate-300 text-white flex justify-between grid-cols-4 gap-2 relative h-[35px] w-[165px] overflow-hidden">
                        @endif
